...
///Field `THOLD1` reader - DAC channel1 hold time (only valid in Sample and hold mode) Hold time= (THOLD\[9:0\]) x LSI clock period Note: This register can be modified only when EN1=0.
...
///Field `THOLD1` writer - DAC channel1 hold time (only valid in Sample and hold mode) Hold time= (THOLD\[9:0\]) x LSI clock period Note: This register can be modified only when EN1=0.
...
///Field `THOLD2` reader - DAC channel2 hold time (only valid in Sample and hold mode). Hold time= (THOLD\[9:0\]) x LSI clock period Note: This register can be modified only when EN2=0. These bits are available only on dual-channel DACs. Refer to implementation.
...
///Field `THOLD2` writer - DAC channel2 hold time (only valid in Sample and hold mode). Hold time= (THOLD\[9:0\]) x LSI clock period Note: This register can be modified only when EN2=0. These bits are available only on dual-channel DACs. Refer to implementation.
