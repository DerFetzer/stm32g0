...
///Falling edge event pending for configurable line
pub type FPIF20_A = FPIF0_A;
///Field `FPIF20` reader - Falling edge event pending for configurable line
pub type FPIF20_R = FPIF0_R;
///Falling edge event pending for configurable line
pub type FPIF20_AW = FPIF0_AW;
///Field `FPIF20` writer - Falling edge event pending for configurable line
pub struct FPIF20_W<'a> {
    w: &'a mut W,
}
impl<'a> FPIF20_W<'a> {
    ///Writes `variant` to the field
    #[inline(always)]
    pub fn variant(self, variant: FPIF20_AW) -> &'a mut W {
        self.bit(variant.into())
    }
    ///Clears pending bit
    #[inline(always)]
    pub fn clear(self) -> &'a mut W {
        self.variant(FPIF20_AW::CLEAR)
    }
    ///Sets the field bit
    #[inline(always)]
    pub fn set_bit(self) -> &'a mut W {
...
impl R {
    ...
    pub fn fpif20(&self) -> FPIF20_R {
        FPIF20_R::new(((self.bits >> 20) & 0x01) != 0)
    }
}
impl W {
    ///Bit 0 - Falling edge event pending for configurable line
    ...
    ///Bit 20 - Falling edge event pending for configurable line
    #[inline(always)]
    pub fn fpif20(&mut self) -> FPIF20_W {
        FPIF20_W { w: self }
    }
