...
///Field `LFTRIG` reader - Low frequency trigger mode enable
...
///Field `LFTRIG` writer - Low frequency trigger mode enable
...
///Field `OVSE` reader - ADC oversampler enable on scope ADC group regular
...
///Field `OVSE` writer - ADC oversampler enable on scope ADC group regular
pub struct OVSE_W<'a> {
    w: &'a mut W,
}
impl<'a> OVSE_W<'a> {
